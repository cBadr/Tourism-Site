...
> ⚠️ **تاريخياً كان جذر git مجلد المستخدم `C:/Users/Badr` لا مجلد المشروع** — أُصلح في 2026-08-12، وتحقَّق `git rev-parse --show-toplevel` في 2026-08-14 أن الجذر الآن هو `Tours-01` نفسه. الخطر القديم (رفع `.ssh` و`.claude.json`) لم يعد قائماً، لكن التحقق قبل أي `git add` يبقى عادة سليمة.
...
| **تليجرام** | ✅ **عاملة بالكامل** (قِيس 2026-08-14): التوكن في `.env.local`، **ومعرّف المحادثة مضبوط** في `site_settings.notifications.telegramChatId`. الأثر: **١٬٧٣٧ إشعاراً `sent`** مقابل ٦ `skipped` وصفر `failed` | ⚠ كان مكتوباً هنا أن المعرّف غير مضبوط وأن الإشعارات «تُتخطّى» — **لم يعد صحيحاً**، أدخله بدر من اللوحة. والمعرّف ليس سرّاً فمكانه اللوحة لا البيئة |
