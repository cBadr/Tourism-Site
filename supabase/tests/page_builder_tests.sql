...
--   (م)   🔴 م‑٧: صورةُ العنصر وأيقونتُه **و`href`** خارج الفهرس وخارج `i18n_apply`،
--         و`alt` **داخله** · والحارس يرفض ثمانية أشكال · والأيقونات مغلقة
...
-- ----------------------------------------------------------------------------
-- (م) 🔴 م‑٧ — وسائط العنصر و`href` **خارج** فهرس الترجمة، و`alt` **داخله**
--
...
begin
  ...
  --
  -- 🔴 **و`href` سادسها منذ `0104` — وهذا التأكيد كان مقلوباً حتى 2026-08-18.**
  --   كان الملفّ يؤكّد عكسه تماماً («`href` يجب ألا يُحجز») محتجّاً بـD-24:
  --   «فالوجهة تختلف بالفعل باختلاف اللغة». **وD-24 لا يقول هذا ولا قريباً منه**
  --   — قُرئ نصّه كاملاً (‏`handover/DECISIONS.md`): «العربية افتراضية وبلا
  --   بادئة؛ `/en/...` **إعادة كتابة داخلية**». أي أن **المسلك واحدٌ للغتين**،
  --   والبادئة يضيفها المصيِّر لا المحتوى: `localeHref` في
  --   `components/site/links.ts` تناديها الكتل الثلاث التي تحمل `href`
  --   (‏`route-rail:197` · `services.tsx:113` · `logo-strip:165`) **بعد**
  --   `i18n_apply` لا قبلها.
  --
  --   **والثمن مقيسٌ لا مُتخيَّل** (2026-08-18): الصفّ
  --   `0b610000-…-0003.items.rtalex.href` حمل ترجمةً آلية
  --   `/routes/cairo-lexandria` — **ألفٌ ابتلعها المترجم** — و`pages` لا تحمل
  --   إلا `cairo-alexandria` ⇒ رابطٌ ميت في أبرز شريط مساراتٍ في الرئيسية،
  --   بلا خطأ ولا سطرٍ في سجل.
  --
  --   🔒 **والسؤال الذي يحسم الاتجاه سُئل وقِيس: هل تختلف الوجهة بلغة؟**
  --   `pages` **بلا عمود لغة أصلاً** — ٢٣ صفّاً · ٢٣ مسلكاً متمايزاً · صفر
  --   عمودٍ اسمُه `%locale%`. فلا توجد «صفحةٌ إنجليزية أخرى» يشير إليها رابطٌ
  --   مترجَم. والاثنا عشر `href` الحيّة كلُّها مسالك داخلية (`/routes/*` ·
  --   `/services/*`)، و`internalPath()` ترفض كل ما لا يبدأ بـ`/` ⇒ **رابطٌ
  --   خارجيّ يختلف بلغة مستحيلٌ بنيوياً هنا، لا مسموحٌ ومهمَل.**
  --
  --   ⚠ **والحارس السلوكيّ للتصيير لا يُستنسخ هنا** (القاعدة الذهبية ١٢):
  --   `i18n_href_tests.sql` القسم (ج) يمرّر خريطةً تحمل `cairo-lexandria`
  --   ويثبت أن `i18n_apply` تُبقي الأصل. وهذا الملفّ يحرس **بابَ الفهرس**.
  foreach v_f in array array['src', 'poster', 'video', 'icon', 'anchor', 'href'] loop
  ...
  foreach v_f in array array['alt', 'imageAlt', 'name', 'label', 'value', 'title', 'text',
                             'num', 'c1', 'h1', 'body', 'note'] loop
    if public.i18n_non_text_field(v_f) then
      raise exception '(م-١) «%» حُجز وهو جملةٌ يقولها إنسان وتُقرأ بلغة الزائر', v_f;
    end if;
  end loop;
  ...
  -- (ج) 🔴 و`href` **خارج** الفهرس منذ `0104` — وهذا التأكيد كان معكوساً،
  --     وتبريرُه القديم («D-24: الوجهة تختلف بالفعل باختلاف اللغة») **لم يكن
  --     في D-24 قطّ**. السبب الحقيقي، بدليل اليوم: المسلك واحدٌ للغتين
  --     (`pages` بلا عمود لغة)، والبادئة `/en` يضيفها `localeHref` عند
  --     التصيير. فدخولُ الرابط الفهرسَ = **طلبُ ترجمةِ مسلك**، وقد وقع فعلاً:
  --     `/routes/cairo-alexandria` ⇐ `/routes/cairo-lexandria`، و`pages` لا
  --     تعرف الثاني ⇒ ٤٠٤ في أبرز شريط مساراتٍ في الرئيسية.
  if exists (
    select 1 from public.i18n_corpus_rows() c
    where c.k = v_sec::text || '.items.m7prb1.href') then
    raise exception '(م-٢ج) `href` دخل فهرس الترجمة — مسلكٌ يُطلب للترجمة، ووجهتُه ٤٠٤';
  end if;
  ...
  raise notice '✔ (م-١..٥) وسائط العنصر والشعار و`href` خارج الفهرس وخارج التطبيق، و`alt`/`name`/`listLabel` داخلها';
end;
$$;
