...
> ## ⛔ قاعدة نشرٍ ملزمة — `README.md` يُحدَّث **قبل** كل نشر
>
> **قرار بدر 2026-08-15، ويسري من النشرة التالية.** لا يُنشَر إصدارٌ قبل أن
> يُحدَّث `README.md` ويُدفَع معه.
>
> **ولماذا قبل لا بعد:** المستودع واجهة المشروع لمن لا يفتح الكود، والنشرُ يغيّر
> ما يعمل فعلاً. فتحديثٌ لاحق يترك نافذةً — قد تطول — يصف فيها GitHub نسخةً غير
> المنشورة. والترتيب `README ← كمّة ← دفع ← نشر` يجعل الاثنين متطابقين **لحظة**
> النشر لا بعده.
>
> **وما يُراجَع في كل مرة — أربعة أسطر لا أكثر:**
>
> | البند | متى يتغيّر |
> |---|---|
> | شارتا `migrations-N` و`SQL suites-N` | مع كل هجرة أو مجموعة اختبار جديدة |
> | «الترحيلات الـ N … إلى `<آخر ملف>`» في قسم البنية | مع كل هجرة |
> | جدول «ما الذي يعمل اليوم» | حين يشحن سطحٌ جديد أو تتغيّر حالة مرحلة |
> | الملخص الإنجليزي | يحمل العددين نفسيهما — يُنسى عادةً، فتحقّق منه |
>
> **والتحقق أمرٌ واحد** يقارن الواقع بما يقوله الملف:
>
> ```bash
...
> قديم — فالأرقام تُقاس والوصف يُقرأ.

> **✅ مُنشأ فعلاً على الخادم منذ 2026-08-15** — ومعه `/etc/sudoers.d/tours`،
> وجُرِّب: `tours` يعيد تشغيل خدمته بلا كلمة مرور. وكان هذا القسم **موثَّقاً
> ولم يُنفَّذ** حتى ذلك اليوم، فكان كل نشرٍ خطواتٍ يدوية — وأخطر ما فيها أن
> ترتيب «الهجرات **قبل** البناء» يعتمد على ذاكرة من ينشر.
>
> **فالنشر اليوم أمرٌ واحد:** `ssh` ثم `/srv/tours/app/deploy.sh`.
>
> ⚠ **وتحذيرٌ لا يُسقَط:** لا قاعدة اختبار منفصلة — القاعدة التي تُطبَّق عليها
> الهجرات أثناء التطوير **هي قاعدة الإنتاج نفسها**. فقد يجد `db:migrate` على
> الخادم «لا جديد» لأن الهجرة طُبِّقت من جهاز التطوير سلفاً؛ وهذا **متوقَّع لا
> عطل**، لكنه يعني أن المخطط يسبق الكود المنشور بين النشرتين. أبقِ الفجوة قصيرة.
