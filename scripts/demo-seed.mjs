    ...
    try {
      ...
      const bk = await one(
        ...
        [
          ...
          b.customerName,
          b.customerPhone,
          // ⚠ **العيب نفسه الذي أُصلح لواتساب المتعهد أعلاه، وبقي هنا وحده.**
          // كان `2${…}` — رمز دولة بخانة واحدة — فيُخرج `21188511418` من
          // `01188511418`. والقاعدة الحية أثبتته: **٣٣٠ من ٣٤١** حجزاً تحمل هذا
          // الشكل، وكل واحد منها يفتح واتساب على «الرقم غير صالح» في بطاقة
          // العميل التي يفتحها المتعهد قبل الانطلاق. والقاعدة الصحيحة `20` ثم
          // الرقم بلا صفره البادئ — وهي قواعد `lib/phone.ts` نفسها.
          // (الصفوف القائمة لا تتغير بهذا السطر؛ يلزمها إعادة بذر.)
          `20${b.customerPhone.slice(1)}`,
          iso(b.pickupAt),
          `${TAG} — ${b.note}`.trim(),
        ]
      );
