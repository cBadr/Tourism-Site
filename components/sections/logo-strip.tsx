...
export async function LogoStripSection({
  ...
  const note =
    ...
    t(
      "note",
      "الشعارات معروضة لبيان طرازات المركبات العاملة في الأسطول فقط، ولا تعني اعتماداً أو علاقة تجارية مع الشركات المصنّعة."
    );
    ...
    <section className="overflow-hidden border-y border-border/60 bg-muted/30 py-10 md:py-14">
      ...
      {note ? (
        <div className="mx-auto mt-8 w-full max-w-3xl px-4 sm:px-6">
          <p className="text-center text-xs leading-6 text-muted-foreground/80">{note}</p>
        </div>
      ) : null}
    </section>
  );
