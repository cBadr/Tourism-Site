 ...
 *
 * ── 🔴 دورُها تغيّر في الدفعة ج (هجرة `0094`) ولم تُحذف ────────────────────
 *
 * الشريط العلوي صار يُبنى من القاعدة (`site_nav` ⇐ `lib/site-nav.ts`)، وهذه
 * الستة **مبذورةٌ صفوفاً في `nav_links`** بنفس المسارات ونفس مفاتيح الرسائل
 * ونفس الترتيب — فالمالك يخفيها ويرتّبها ويضيف إليها من اللوحة.
 *
 * وبقاؤها هنا لسببين لا واحد:
 *
 * ١ **الاحتياطي.** بلا بيئة Supabase أو عند فشل النداء تُصيَّر هذه القائمة
 *   حرفاً (‏`fallbackNav` في `lib/site-nav.ts`) — فالفشل يقع إلى «كما كان
 *   الموقع أمس» لا إلى ترويسةٍ بلا تنقّل.
 *
 * ٢ **التذييل يقرؤها كما كان** (`navLinks()` أدناه): عمود «أقسام الموقع» و«تابع
 *   حجزك» في `footer.tsx` لم يُلمسا. والفرق بين السطحين **مقصود**: التذييل
 *   خريطةٌ كاملة تعرض كل ما يمكن الوصول إليه، والشريط قائمةٌ **منتقاة** بسقفٍ
 *   مقيس. فلو قُرئ السطحان من مصدرٍ واحد لصار إخفاءُ بندٍ من الشريط قطعاً
 *   لطريقٍ داخلي إلى صفحة — وهو بعينه العيب الذي أُصلح في الفجوتين ١٦ و٢٣.
 *
 * ⚠ **ومن يعدّل هذه القائمة اليوم يعدّل الاحتياطي والتذييل فقط** — لا الشريط.
 * الشريط تعدّله بذرةُ `0094` أو المالك من `/admin/content`.
 */
...
/**
 * 🔴 مرآةُ `public.nav_href_reserved(text)` في هجرة `0094` — البند الثالث من
 * بنود بدر: **مدخل حساب العميل ليس بنداً في الشريط العلوي ولا يمكن أن يصير.**
 *
 * والقاعدة هي الحارس الحقيقي (قيدٌ على الجدول ومُشغّلٌ يسمّي السبب)؛ وهذه
 * **للرسالة قبل الرحلة** وحدها: نموذج اللوحة يرفض ويشرح بالعربية قبل أن يرتدّ
 * من الخادم برسالةٍ عامة. ولو انحرفت النسختان فالقاعدة تكسب — وهذا هو الاتجاه
 * الصحيح للانحراف (تشدّدٌ في الحارس، لا تسامحٌ في الواجهة).
 *
 * وتقبل بادئة اللغة أمامها (`/en/account/login`) والاستعلام والمرساة بعدها،
 * لأن `nav_href_reserved` تقبلهما — والصيغتان يجب أن تحكما على النصّ نفسه.
 */
const ACCOUNT_RESERVED_RE = /^\/(?:[a-z]{2}(?:-[a-z]{2})?\/)?account(?:[/?#]|$)/;

/**
 * أقصى طول تسميةٍ في الشريط — مرآةُ `pages_nav_label_shape` و
 * `nav_links_label_check` في `0094`.
 *
 * ⚠ **وليس هذا سقفَ التصميم**: سقفُ التصميم **عددُ** البنود (`nav_cap()` = ٧)،
 * وهو محسوبٌ في القاعدة ومعروضٌ تحذيراً. وهذا الرقم حرسٌ من نوعٍ آخر — ألّا
 * يُلصق في خانة «الاختصار» عنوانٌ كامل فيفقد الحقل غرضه («الشروط» لا «الشروط
 * والأحكام والسياسات المرتبطة بها»). فهو **سخيٌّ بقصد**: يمنع الجملة ولا يجادل
 * المالك في كلمتين.
 */
...
/**
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║  هالةُ لمسٍ ٤٤×٤٤ — مساحةٌ تُنقر أوسعُ من الصندوق الذي يُرى                ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
 *
 * ── القياس الحيّ الذي أوجبها (٢٠٢٦‑٠٨‑١٨ · الشريط العلوي · خادم :3000) ────
 *
 * | الهدف | المقيس بكسل | ٤٤×٤٤؟ |
 * |---|---|---|
 * | «احجز الآن» | ٩٩×٣٦ | ✗ |
 * | «حسابي» | ١١٢٫٥×٣٨ | ✗ |
 * | حبّة اللغة «EN» | ٦٧٫٥×٣٨ | ✗ |
 * | روابط التنقّل | ٦١–١٢٥ × ٣٦ | ✗ |
 * | زرّ القائمة ☰ | ٣٦×٣٦ | ✗ |
 * | الشعار | ١٥٤٫٨×٣٦ | ✗ |
 *
 * **ولا هدفَ واحدٌ منها كان يبلغ ٤٤** — بينما شريط الإجراء السفلي
 * (`cta-bar.tsx`) يبلغها منذ يومه الأول (`h-12` = ٤٨). أي أن المنتج كان
 * يحمل مقياسين للّمس في صفحةٍ واحدة، وأصغرُهما على الشريط الذي يُنقر أولاً.
 *
 * ── ولماذا هالةٌ لا تكبيرُ الحبّة نفسها ────────────────────────────────────
 *
 * لأن الحلّ المباشر ينقض القرار الذي بُني عليه هذا الشريط:
 *
 * | لو رُفع الصندوق نفسه إلى ٤٤ | العاقبة المقيسة |
 * |---|---|
 * | مبدّل اللغة ٤٤ و«احجز الآن» ٣٦ | **الثانويّ يصير أطول من نداء المنتج الأول** — نقضُ «الفصل بالوظيفة» بعينه |
 * | ورفعُ الاثنين معاً | صفُّ تحكّمٍ ٤٤ داخل شريطٍ ارتفاعه ٦٤ ⇒ زحمةٌ أشدّ من التي شكا منها بدر |
 * | وزرّ ☰ من ٣٦ إلى ٤٤ عرضاً | عند ٣٧٥ لا يبقى في الصفّ إلا **١٦٫٥ بكسل حرّة** (مقيسة) — فالثمانية الزائدة تكاد تستهلكها كلَّها |
 *
 * فالهالة تفصل **ما يُرى** عن **ما يُنقر**: الصندوق يبقى كما قِيس — فلا تتغيّر
 * أرقام كتلة اليمين (٢٩٥ عربية · ٣١٥ إنجليزية) المكتوبة في `header.tsx` —
 * و`::after` تمدّ مساحة النقر إلى ٤٤ بلا أن تزيح بكسلاً، لأنها `absolute`
 * أي خارج التدفّق.
 *
 * ⚠ **والتمدّد رأسيٌّ وحده في `ROW`** بقصد: الفجوة الأفقية بين حبّات الشريط
 *   ٨ بكسل (`gap-2`) وبين روابط التنقّل ٤ (`gap-1`) — فتمدّدٌ أفقيّ يجعل
 *   هالةَ حبّةٍ تسرق نقرةَ جارتها، **وهو عيبٌ لا يظهر في لقطة شاشة** ولا يشكو
 *   منه إلا من أخطأ هدفه فوصل غيرَه. والرأسيّ آمنٌ: الشريط ٦٤ والحبّة ٣٦–٣٨،
 *   فيبقى نحو ١٣ بكسل فوقها ومثلها تحتها.
 *
 * ⚠ **و`SQUARE` للمربّعات وحدها** (زرّ ☰ ‏٣٦×٣٦): يتمدّد ٤ بكسل في كل اتجاه.
 *   وقِيس أن الفجوة إلى «احجز الآن» ٨ ⇒ يبقى بينهما ٤ بكسل خالية فلا تتلامسان.
 *
 * 🔒 **ولا تُستعمل أيٌّ منهما داخل قائمةٍ رأسية** (الدرج · لوح الحساب · صفوف
 * المظهر): هناك سعةٌ رأسية فالصواب حشوٌ **حقيقيّ** (`py-3` ⇒ ٤٤) لا هالة —
 * وهالتان متجاورتان في صفٍّ رأسيٍّ فجوتُه ٤ بكسل **تتداخلان** فتبتلع إحداهما
 * نقرة الأخرى. **القاعدة: هالةٌ حيث الضيق، وحشوٌ حيث السعة.**
 */
export const TAP_TARGET_ROW =
  "relative after:absolute after:inset-x-0 after:top-1/2 after:h-11 after:-translate-y-1/2 after:content-['']";

/** هالةٌ مربّعة ٤٤×٤٤ — للأزرار الأيقونية وحدها (المبرر في `TAP_TARGET_ROW` أعلاه) */
export const TAP_TARGET_SQUARE =
...
/** هل هذا الرابط مدخلَ حساب؟ (‏مرآة `nav_href_reserved` — انظر التعليق أعلاه) */
export function isAccountReservedHref(href: string): boolean {
  return ACCOUNT_RESERVED_RE.test(href.trim());
}

/**
 * هل يصلح هذا النصّ رابطاً لبند شريط؟ مرآةُ `public.nav_href_ok(text)`.
 * مسارٌ داخلي بشرطةٍ واحدة (`/`، `/book`، `/#services`) أو عنوانٌ مطلق http(s).
 * و`//host` مرفوضٌ: يخرج بالزائر من الموقع بلا أن يبدو خارجياً.
 */
export function isNavHrefShapeOk(href: string): boolean {
  const value = href.trim();
  if (value !== href) return false;
  return value === "/" || /^\/[^/]/.test(value) || /^https?:\/\/[^/]/.test(value);
}
