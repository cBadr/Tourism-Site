...
const OUTCOME_STYLE: Record<Outcome, { box: string; icon: string }> = {
  ...
  failed: {
    ...
    /*
      🔴 `text-background` لا `text-destructive-foreground` — لسببين مقيسين:

      (١) **الثاني لا وجود له.** `--color-destructive-foreground` غير مسجَّل في
          كتلة `@theme` في `globals.css` (المسجَّل `--color-destructive` وحده)،
          فالصنف لا يولّد قاعدةً أصلاً والأيقونة كانت ترث `--foreground`. صنفٌ
          ميّت يبدو حيّاً.

      (٢) **وأثره ينقلب مع الأرضية.** `--destructive` **يُشتقّ لقراءة النصّ لا
          للملء**: في العائلة الفاتحة `--danger-on-sand` (أحمر غامق) وفي الداكنة
          `--danger-on-ink` (أحمر فاتح). فحين صارت هذه الصفحة داكنة في م‑٨ صار
          الرمزُ الفاتح الموروث على أحمرَ فاتح — **٢٫٥٧:١ مقيسة**.

      و`--background` هو الرمز الوحيد الذي ينقلب **في الاتجاه نفسه**: حبريٌّ فوق
      الأحمر الفاتح (‏٦٫٦٢:١ مقيسة)، ورمليٌّ فوق الأحمر الغامق (‏٤٫٦٩:١) — فيصحّ
      في العائلتين بلا لونٍ مكتوب، ويصحّ يوم يأتي مبدّل م‑٩ بلا لمسه.
    */
    ...
    case "succeeded":
      return {
        ...
        body: t(
          ...
          "وصلنا تأكيد البنك واعتُمد المبلغ، وانتقل حجزك إلى «مؤكد». نتواصل معك قبل الموعد بتفاصيل السيارة والسائق."
        ),
      };
    case "waiting":
      return {
