 ...
 *
 * ⚠ **وهي مفردات شاشة «حجوزاتي» لا مخرجات هذين الإجراءين وحدهما**: الشاشة تبني
 * `Record<AccountNoticeCode, …>` عليها، فكل من يهبط عليها برمزٍ يجب أن يكون فيها.
 * ويهبط عليها ثلاثة: هذان الإجراءان، و`linkBookingByToken` في `../actions.ts`،
 * **ومهبطُ رابط تأكيد البريد** (`../callback/route.ts`) — وهذا الأخير هو الذي
 * كشف أن الاتحاد كان أضيق من مستهلكه: كان يُرسل `?done=confirmed` إلى هذه
 * الشاشة وهي لا تقرأ `done` أصلاً، فيصل العميل بعد تأكيد بريده إلى قائمته
 * **بلا سطر واحد يقول له إن التأكيد نجح** — وهو بالضبط عيب «رمزٌ بلا رسالة»
 * الذي تُبنى هذه الخريطة لمنعه. (وتأكيدات البريد **مفعّلة** في هذا المشروع —
 * مقيسة على `auth.users`: صفٌّ بـ`confirmation_sent_at` و`email_confirmed_at`
 * فارغ — فالمسار حيٌّ لا نظري.)
 */
export type AccountNoticeCode = "linked" | "already-linked" | "confirmed";
