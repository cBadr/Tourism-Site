...
/**
 * 🔒 **المصدر الوحيد لتسميات الحالة** — تُقرأ من هنا ولا تُكتب حرفياً في أي شاشة.
 * وتبويبات `/admin/orders` تقرؤها من هذا القاموس لا تنسخه: النسخةُ هي ما جعل
 * `failed` تتخلّف عن أخواتها الست حين طُبِّق نمط «تم + مصدر»، فبقيت وحدها صفةً.
 *
 * والنمط قرارُ المالك (2026-08-17): **فعلٌ وقع ⇒ «تم + مصدر»**، ومنفيُّه نفيُ
 * المصدر لا صفة. فـ`failed` هي نقيض `completed: "تم التنفيذ"` ⇒ «لم يتم التنفيذ»
 * — وهو ما يقوله للعميل سطحُه العام سلفاً (`bookingStatus.titleFailed` =
 * «رحلة لم تُنفَّذ»)، وما تقوله جلسةُ الدفع الفاشلة («لم يتم الدفع») وطابورُ
 * الإشعارات («لم يتم الإرسال»). ⚠ والمعرِّف المخزَّن `failed` لا يُمسّ.
 */
