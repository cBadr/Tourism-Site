...
const SAVED_MESSAGES: Record<string, string> = {
  ...
  trip: "حُفظت إعدادات الرحلات — المهلة الجديدة سارية من الكنس التالي (المجدول أو اليدوي)، والمنطقة الزمنية سارية فوراً على العرض وعلى تفسير المواعيد الجديدة.",
};
...
export default async function SettingsPage({ searchParams }: PageProps<"/admin/settings">) {
  ...
  const errorMessages: Record<string, string> = {
    ...
    timezone:
      "المنطقة الزمنية غير معروفة — اخترها من القائمة. القائمة مأخوذة من قاعدة المناطق التي يشغّلها الخادم نفسه، فلا يُقبل اسم مكتوب بيد.",
    timezonedb:
      "رفضت قاعدة البيانات اسم المنطقة الزمنية — يعرفه المتصفح ولا تعرفه نسخة Postgres. اختر اسماً آخر من القائمة (مثل Africa/Cairo أو Asia/Riyadh).",
    tripnotready:
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <form action={saveSettings} className="space-y-6">
        ...
        {/*
          ╔══════════════════════════════════════════════════════════════════╗
          ║  لوحة المظهر الفاتح (م‑٩) — بطاقةٌ مستقلة لا تبويبٌ ولا طيّة       ║
          ╚══════════════════════════════════════════════════════════════════╝

          ── ولماذا **مستقلة** ────────────────────────────────────────────────

          لأن اللوحتين تُقرآن بالمقارنة لا بالتتابع: من يضبط الكهرمان الفاتح
          يحتاج أن يرى الكهرمان الداكن في نفس التمرير. وتبويبٌ يخفي إحداهما
          يجعل الضبط ذهاباً وإياباً بلا مرجع.

          🔴 **وأخطر ما في هذه الشاشة ليس اللون بل التوقّع**: المالك الذي يبدّل
          «لون الفعل» في البطاقة الأولى يفترض أنه بدّل لون الموقع — وقد بدّل
          نصفه. ولهذا الجملة أدناه صريحةٌ لا مجاملة، والتسميات هي هي في
          البطاقتين (‏`COLOR_GROUPS` مصدرٌ واحد) كي يقع الفرق على العنوان وحده.

          ⚠ ولا يوجد زر «اشتق الفاتحة من الداكنة». المحاولة الأولى في هذا
          المشروع بُنيت على الاشتقاق (`0063`) وأسقطت هوية التصميم كلها بلونٍ
          واحد؛ والقاعدة المستخلصة أن **درجة اللون فوق أرضيةٍ ما ليست دالّةً في
          درجته فوق الأخرى**. فالاشتقاق هنا كان سيعرض على المالك رقماً يبدو
          محسوباً وهو ليس مقيساً، وذلك أسوأ من حقلٍ يملؤه بيده.
        */}
        <Card className="space-y-4 p-5">
          <div>
            <h2 className="flex items-center gap-1.5 font-heading text-base font-bold">
              لوحة المظهر الفاتح
              <HelpTip>
                الزائر يبدّل بين المظهرين من ترويسة الموقع، والافتراضي عند من لم يختر هو مظهر
                جهازه. فهذه اللوحة يراها **نصف الزوار تقريباً** ولا تقلّ أهمية عن الأولى.
                وهي مستقلة عنها تماماً: لا يُشتقّ لونٌ هنا من لونٍ هناك.
              </HelpTip>
            </h2>
            <p className="text-sm leading-relaxed text-muted-foreground">
              سبعة عشر لوناً أخرى — نفس الأدوار، مضبوطةً للأرضية الفاتحة. ولا تُنسخ من اللوحة
              الداكنة: لونٌ يُقرأ فوق الحبر قد يختفي فوق الرمل. (كهرمان المظهر الداكن فوق أرضية
              فاتحة يعطي ‎2.14:1‎، والحد المطلوب ‎4.5:1‎.)
            </p>
            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
              وثلاثة أدوار تنقلب هنا عمداً ولا تُصحَّح: <strong>النص فوق لون الفعل</strong> يصير
              فاتحاً لا داكناً، و<strong>درجة الفعل الأفتح</strong> تصير أغمق لا أفتح (التمرير
              يُبلَغ على الفاتح بالإعتام)، و<strong>أرضية شارة المعلومة</strong> تصير درجة فاتحة
              لا داكنة.
            </p>
          </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {group.fields.map(([key, label]) => (
                  <ColorField
                    key={key}
                    label={label}
                    name={`brand.colorsLight.${key}`}
                    defaultValue={settings.brand.colorsLight[key]}
                    disabled={!wired}
                  />
                ))}
              </div>
