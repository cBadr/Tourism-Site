...
/**
 * الهامش الحقيقي للرحلة = إجمالي الحجز − مستحق المتعهد المنفِّذ.
 *
 ...
 *
 * لماذا الطرح في الواجهة لا في SQL: العددان مخزَّنان جاهزين في جدولين مختلفين
 * (`bookings.total` و`dispatches.assigned_payout`) وPostgREST لا يُجري عمليات
 * حسابية بينهما، وهجرة المرحلة ٦ ليست ملك هذه الشاشة. وهذا **عرض لا قرار**:
 * الرفض الملزِم عند نزول الهامش تحت أرضيته يقع داخل `manual_assign` في قاعدة
 * البيانات، وما هنا تحذير مبكر يمنع الضغطة الخاطئة. متى أضافت الهجرة عموداً
 * محسوباً للهامش فاقرأه بدل هذا الطرح.
 */
export function realMargin(total: number | null, payout: number | null): number | null {
