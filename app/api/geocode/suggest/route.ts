...
export async function POST(request: NextRequest) {
  ...
  /**
   * 🔴 **الفشل والفراغ لا يخرجان من هذا الباب بحالةٍ واحدة.**
   *
   *   • `ok: true` بصفر اقتراح ⇒ ٢٠٠. «بحثنا في كل ما نملك ولم نجد» جوابٌ
   *     صحيح لا عطل، والواجهة تعرض عليه مخارج الطبقتين ٣ و٤.
   *   • `unavailable` ⇒ **٥٠٣ لا ٤٠٠**: لم يُجب مزوّدٌ واحد (مفتاح · حصّة ·
   *     شبكة)، وهذا عطلٌ عندنا لا خطأٌ في طلب العميل. و٤٠٠ كان سيكذب على كل
   *     مراقبةٍ تقرأ رموز الحالة، ويخلط انقطاع مزوّدٍ بنصٍّ مشوَّه.
   *   • `too-short` / `invalid-input` ⇒ ٤٠٠، وهي فعلاً في الطلب.
   *
   * ⚠ والواجهة لا تنكسر بأيٍّ منها: `place-field.tsx` يقرأ الجسم أياً كانت
   *   الحالة، و`json.ok === false` تفتح المخارج — فالعميل أمام الخريطة وطلب
   *   عرض السعر في الحالتين، والفرق **ما يُقال له** لا ما يستطيعه.
   */
  if (result.ok) return json(result, 200);
  return json(result, result.code === "unavailable" ? 503 : 400);
}
