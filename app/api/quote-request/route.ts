 ...
 * ومحدَّثة في 0009): تُدرج الطلب وتُرجع رقمه المرجعي بلا منح الزائر أي SELECT
 * على الجدول، وتتحقق من المدخلات داخل القاعدة (طول الاسم، أرقام الهاتف، سقف
 * التفاصيل، الخدمة من القائمة المعروفة) بحيث تكون الحراسة في مكان واحد.
 *
...
/**
 * رسائل رفض الدالة الحارسة — الرمز يأتي في `hint` (هجرة 0009 ترفع
 * `invalid-input`؛ التفصيلات مذكورة هنا تحسّباً لتوسيع الرموز لاحقاً).
 */
...
export async function POST(request: Request) {
  ...
  if (details.length < MIN_DETAILS_LENGTH) {
    return errorJson(
      "invalid-input",
      "اكتب تفاصيل طلبك: الوجهة والتاريخ وعدد الأفراد وأي ملاحظات.",
      400
    );
  }
