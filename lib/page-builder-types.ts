...
export const BLOCK_CATALOGUE: readonly BlockDef[] = [
  ...
  // ── كتل م‑٢ الثلاث — تركيب التصميم، ولا رابعة ─────────────────────────
  {
     ...
     *
     * 🆕 **و`listLabel` أُضيفت 2026-08-17 (`0101`) — وهي إصلاحُ حقلٍ كان خارج
     * اللوحة كلياً، لا حقلٌ جديد يُطلب.**
     *
     * كانت تسمية `<ul>` الشريط لقارئ الشاشة محفورةً في العارضة
     * (`t("listLabel", "ماركات الأسطول")`) **بلا مسار `content` إطلاقاً** —
     * فقيمتها تُصيَّر على الموقع الحيّ ولا تُغيَّر من اللوحة ولا من القاعدة. وهو
     * نقضٌ للشرط غير القابل للتفاوض («التحكم في كل شيء من لوحة التحكم»)،
     * **وحملُها بعينه الصياغة التي كُتبت `0095` لإزالتها**: النشاط وسيطٌ لا يملك
     * مركبةً، فـ«الأسطول» بضمير الملك ادّعاءُ ما لا يُملَك. و`0095` نظّفت القاعدة
     * ولم تجد لهذه التسمية صفّاً، لأنها لم تكن في القاعدة.
     *
     * وموضعها **بين `title` و`note` لا في الذيل**: ترتيب `textFields` هو ترتيب
     * الخانات في اللوحة (`BlockFields` تحلق عليه كما هو)، وتسميةُ القائمة تخصّ
     * العنوان لا الملاحظة التحريرية أسفل الشريط.
     */
    type: "logo-strip",
    role: "system",
    placement: "once-per-page",
    acceptsChildren: false,
    maxChildren: null,
    textFields: ["title", "listLabel", "note"],
    /**
