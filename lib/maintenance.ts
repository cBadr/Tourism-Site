...
/**
 * هل يحمل الطلب كوكي جلسة Supabase؟ يُستخدم مع allowAdmin ليتصفح الفريق
 * الموقع العام أثناء الصيانة. تنبيه: هذه بوابة تجربة لا بوابة أمان — التحقق
 * الحقيقي من الدور يبقى في RLS وفي حارس /admin.
 */
...
export function maintenanceContactLinks(
  ...
  if (state.contact.whatsapp) {
    links.push({
      href: `https://wa.me/${state.contact.whatsapp.replace(/[^\d]/g, "")}`,
