 ...
 * | `/admin/loyalty` | **له تدفّق حقيقي** (نقاط تُسكّ وتُستبدل يومياً) — والمانع أن `pulse_stats`/`pulse_series` (‏0034 و0035) سبقتا نظام الولاء (‏0047) فلا قسم `loyalty` فيهما أصلاً؛ قِيس على القاعدة الحيّة لا استُنتج. **بند مؤجَّل بوعي كـ`/admin/logs` لا استثناء بقرار**، ومُحفِّزه أول هجرة تضيف قسماً إحصائياً. ولا يُقاس هنا في TypeScript بحال (**D-05**) — وأخصّه الالتزام القائم، فهو مبلغٌ مستحق على المنصة لا عدّاد صفوف |
