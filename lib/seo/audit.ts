...
export function auditBusiness(settings: SiteSettings): BusinessField[] {
  ...
  return [
    ...
    },
    {
      ...
      note: "الشعار عقدة صورة واحدة تشير إليها المؤسسة وبطاقة النشاط معاً — وبدونه تخرج النتيجة بلا أيقونة. وهو للترويسة والبيانات المهيكلة فقط، ولا يُستعمل بطاقةَ مشاركة (انظر السطر التالي).",
    },
    {
      /**
       * ⚠ صفٌّ **جديد لأن الشاشة كانت صامتة عنه تماماً** — لا تكذب ولا تُنبّه.
       * وكان الموقع يشارَك بلا صورة على كل صفحة: الشعار (‏١٤١×٤٩ مقيساً) كان
       * يزحزح ملف `og-default.png` ‏(‏١٢٠٠×٦٣٠) من سلسلة المرشّحين، فتقيسه
       * المنصة وترفضه لأنه دون حدّها الأدنى.
       *
       * و`present` هنا تعني «المالك ضبط صورةً خاصة» لا «توجد صورة»: الاحتياطي
       * قائم دائماً وصالح، ولذلك الدرجة `info` لا `warn` — النقص تحسينٌ لا عطل.
       * **والمقاس لا يُفحص هنا**: قياسه يستلزم جلب الملف، وهذه دالة متزامنة
       * تُنادى في كل تصيير. موضع الفحص الصحيح **لحظة الحفظ** في شاشة الإعدادات،
       * وهو بندٌ مفتوح لا يدّعي هذا السطر أنه أُنجز.
       */
      key: "ogImage",
      label: "صورة المشاركة",
      group: "identity",
      severity: "info",
      present: set(settings.seo.ogImageUrl),
      note: "الصورة التي تظهر حين يُشارَك رابط من الموقع على واتساب أو فيسبوك. بلا ضبطٍ خاص تُستعمل صورة المشروع الافتراضية ١٢٠٠×٦٣٠ وهي صالحة. وإن ضبطتها فاجعلها ١٢٠٠×٦٣٠ — ما دون ٢٠٠×٢٠٠ تتجاهله فيسبوك فتخرج البطاقة بلا صورة.",
    },
    {
      key: "legalName",
