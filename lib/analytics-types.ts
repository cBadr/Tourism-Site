 ...
 *   v_stats_treasury      — الخزينة: أرصدة وتدفق نقدي بالفترة
 *   v_stats_customers     — العملاء: طلبات الفترة والعائد والتكرار
 ...
 *       — القمع يومياً من بيانات المنصة نفسها (لا من جوجل)
 *   funnel_summary(p_from date, p_to date)   ← أُضيفت في 0023
 *       — إجمالي كل مرحلة ومعدل تحولها، بعمود in_chain يفصل السلسلة عن الجانبيين
 *   funnel_counts(p_from date, p_to date)    ← داخلية، غير ممنوحة لأي دور
 *       — مصدر العدّ الوحيد للدالتين معاً، فيستحيل أن يفترق مجموع الرسم عن رقم البطاقة
 *
 * ── نموذج القمع بعد الهجرة 0023 (صُحِّح فيها، فلا تُعِد نموذج 0022) ──────────
 *
 * **سلسلة رباعية ومصدر واحد:** بحث ← عرض ← حجز ← دفع، كلها من `funnel_events`
 * وحده. كان القاع يُقرأ من `bookings` و`payments` فكان كل حجز يُدخله فريق التشغيل
 * أو سبق وجود القياس يُحسب بلا رأس ⇒ معدل تحول يتجاوز ١٠٠٪.
 *
 * **حدثان خارج السلسلة بلا نسبة متسلسلة:** `quote_requested` مسار دخول موازٍ،
 * و`booking_started` لا يقع إلا على مسار البوابة الإلكترونية والافتراضي تحويل بنكي.
 *
 * **نسبة التحصيل كوهورت** — تحصيلات الحجوزات التي أُنشئت داخل النافذة نفسها —
 * فهي ≤١٠٠ بنيوياً لا بسقف. (سقفٌ على القيمة يجعل الفحص الذي يحرسها زينةً.)
 *
 * **والقمع يقيس الرحلة على الموقع لا إجمالي أعمال الشركة.** حجز هاتفي لا يظهر فيه،
 * وهذا صحيح لا نقص؛ الأرقام المرجعية مكانها قسم الطلبات وقسم الخزينة.
 */
